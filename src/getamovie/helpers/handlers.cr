...
class Kemal::RouteHandler
  {% for method in %w(GET POST PUT HEAD DELETE PATCH OPTIONS) %}
    exclude ["/api/v1/*", "/api/watch_party", "/api/experimental"], {{method}}
  {% end %}
...
class Kemal::ExceptionHandler
  ...
  private def call_exception_with_status_code(context : HTTP::Server::Context, exception : Exception, status_code : Int32)
    ...
    if !Kemal.config.error_handlers.empty? && Kemal.config.error_handlers.has_key?(status_code)
      context.response.content_type = "text_html" unless context.response.headers.has_key?("Content-Type")
...
class FilteredCompressHandler < Kemal::Handler
  exclude ["/api/v1/auth/notifications", "/api/watch_party", "/api/experimental"]
  exclude ["/api/v1/auth/notifications", "/data_control", "/api/watch_party", "/api/experimental"], "POST"
...
class DenyFrame < Kemal::Handler
  exclude ["/embed/*", "/api/watch_party", "/api/experimental"]
